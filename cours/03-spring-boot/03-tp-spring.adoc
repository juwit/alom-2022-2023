...
Identifiez vous sur Github, et cliquez sur le lien suivant pour créer votre repository git: https://classroom.github.com/a/zccBCv1X[Github classroom,window="_blank"]
...
Cette structure est une structure que l'on retrouve très souvent sur les projets d'entreprise.
====
...
Nous allons utiliser maintenant configurer `spring-boot`.

Ajoutez le bloc suivant (en haut de votre `pom.xml`) :
...
Ajoutez également la dépendance `spring-boot-starter-web` :
...
Supprimez la dépendance à `jackson-databind` qu'on avait ajouté précédemment.
Nous n'en avons plus besoin car `spring-boot-starter-web` l'importe pour nous !
